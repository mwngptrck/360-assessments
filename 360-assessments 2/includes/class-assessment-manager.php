...
class Assessment_360_Assessment_Manager {
    ...
    public function get_dashboard_stats() {
        ...
        if ($assessment) {
            // Try to get count from relationships table first
            $total = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}360_user_relationships WHERE assessment_id = %d",
                $assessment->id
            ));
            $total = intval($total);

            // If relationships table is empty, fallback to unique pairs in responses table
            if ($total === 0) {
                $total = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(DISTINCT assessor_id, assessee_id) FROM {$wpdb->prefix}360_assessment_responses WHERE assessment_id = %d",
                    $assessment->id
                ));
                $total = intval($total);
            }
            $stats->total_assessments = $total;

            // Completed: count unique (assessor_id, assessee_id) pairs marked as completed
            $completed = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(DISTINCT assessor_id, assessee_id) FROM {$wpdb->prefix}360_assessment_responses WHERE assessment_id = %d AND status = 'completed'",
                $assessment->id
            ));
            $stats->completed_assessments = intval($completed);

            // Completion %
            $stats->completion_rate = $stats->total_assessments > 0 ? round(($stats->completed_assessments / $stats->total_assessments) * 100) : 0;

            // Active users for this assessment
            $stats->active_users = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(DISTINCT assessor_id) FROM {$wpdb->prefix}360_assessment_responses WHERE assessment_id = %d",
                $assessment->id
            ));
        }

        // Count active and archived assessments
        $stats->active_assessments = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}360_assessments WHERE status = 'active'");
        $stats->archived_assessments = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}360_assessments WHERE status = 'archived'");
