...
// 1. Get all groups in this assessment (with peers aggregation)
$all_groups = $data_manager->get_user_groups_for_assessment($assessment_id, $user_id);
// 2. Get all assessors count per group
...
// 6. All individual responses (anonymous)
...
$summary_intro = "This report presents the feedback gathered from all assessors during the assessment period. The results are grouped by user type and summarized with graphical and tabular representation, as well as detailed individual responses.";
?>
<div class="wrap">
    ...
    <div class="mb-4">
        ...
        <table class="wp-list-table widefat fixed striped compact-results-table">
            <thead>
                <tr>
                    <?php foreach ($assessor_counts as $g => $count): ?>
    ...
    <div class="mb-4">
        ...
        <table class="wp-list-table widefat fixed striped compact-results-table">
            <thead>
                <tr>
                    <?php foreach ($group_averages as $g => $avg): ?>
                        <th><?php echo esc_html($g); ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <?php foreach ($group_averages as $g => $avg): ?>
                        <td><?php echo is_numeric($avg) ? number_format($avg, 2) : ''; ?></td>
                    <?php endforeach; ?>
                </tr>
            </tbody>
        </table>
