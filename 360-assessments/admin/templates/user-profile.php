...
try {
    ...
    if (!isset($user->group_id) || !isset($user->group_name)) {
        if (WP_DEBUG) {
            error_log('Warning: User #' . $user_id . ' has no group assigned');
        }
    ...
    if (WP_DEBUG) {
        ...
        $relationships = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}360_user_relationships WHERE assessor_id = %d",
            $user_id
        ));
        error_log("Found " . count($relationships) . " relationships");
        error_log("Relationships: " . print_r($relationships, true));
        
        // Debug assessees
        error_log("Found " . count($assessees) . " assessees");
        if (!empty($assessees)) {
            error_log("Assessees data: " . print_r($assessees, true));
        }
    }
?>
...
<div class="wrap">
    ...
    <div class="mb-4 d-sm-flex align-items-center">
        <h1 class="wp-heading-inline h3 mb-3 mb-sm-0">
            <i class="bi bi-person-circle me-2"></i>User Profile for: <span><?php echo esc_html($user->first_name . ' ' . $user->last_name); ?></span>
        </h1>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <div class="text-center mb-4">
                        ...
                        <h3 class="h4 mb-1"><?php echo esc_html($user->first_name . ' ' . $user->last_name); ?></h3>
                                        ...
                                        <table class="table table-hover align-middle mb-0">
                                            ...
                                            <tbody>
                                                <?php foreach ($assessments as $assessment): 
                                                    if (WP_DEBUG) {
                                                        error_log("Processing assessment: " . print_r($assessment, true));
                                                    }
                                                ?>
                                                    <tr>
                        ...
                        <div class="card shadow-sm" style="margin-top: 20px;">
                            <div class="card-header bg-light">
                                <h2 class="card-title h5 mb-0">
                                    ...
                                    <?php 
                                    if (WP_DEBUG) {
                                        error_log('Checking assessees count');
                                        error_log('Assessees: ' . print_r($assessees, true));
                                    }
                            ...
                            <div class="card-body">
                                <?php 
                                if (WP_DEBUG) {
                                    error_log('Assessees data before display:');
                                    error_log(print_r($assessees, true));
                                }
                                                ...
                                                <?php foreach ($assessees as $assessee): ?>
                                                    <tr>
                                                        ...
                                                        <td class="text-end">
                                                            <?php
                                                            if (WP_DEBUG) {
                                                                error_log("Getting status for assessor {$user_id} and assessee {$assessee->id}");
                                                            }
                                                            $status = $assessment_manager->get_assessment_status(
                                                                $user_id,
                                                                $assessee->id,
...
<!--
                                                        <td class="text-end">
                                                            <?php if ($status !== 'completed' && $current_assessment): ?>
