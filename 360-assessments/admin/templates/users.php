...
try {
    ...
    if ($current_action === 'edit' || $current_action === 'new') {
        ...
        try {
            ...
            throw new Exception('Error loading form data: ' . $e->getMessage());
        }
    }
            ...
            <form method="post" action="<?php echo admin_url('admin-post.php'); ?>" class="validate">
                ...
                <?php if (WP_DEBUG): ?>
                    <div class="notice notice-info inline">
                        ...
                        <ul>
                            <li>Action: <?php echo esc_html($current_action); ?></li>
                            <li>User ID: <?php echo $user ? esc_html($user->id) : 'New User'; ?></li>
                            <li>Total Positions: <?php echo count($positions); ?></li>
                            <li>Total Groups: <?php echo count($groups); ?></li>
                            <li>Potential Assessors: <?php echo count($potential_assessors); ?></li>
                            <li>Current Assessors: <?php echo count($current_assessor_ids); ?></li>
                        </ul>
                    </div>
                <?php endif; ?>
                        ...
                        <div id="assessors-section" class="assessors-section">
                            ...
                            <?php if (WP_DEBUG): ?>
                                <div class="notice notice-info inline">
                                    <p>Debug Information:</p>
                                    <ul>
                                        <li>Total Potential Assessors: <?php echo count($potential_assessors); ?></li>
                                        <li>Current Assessors: <?php echo count($current_assessor_ids); ?></li>
                                    </ul>
                                </div>
                            <?php endif; ?>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td class="column-email"><?php echo esc_html($user->email); ?></td>
                            <td class="column-position"><?php echo esc_html($user->position_name ?? '—'); ?></td>
                            <td class="column-group"><?php echo esc_html($user->group_name ?? '—'); ?></td>
...
jQuery(document).ready(function($) {
    ...
    function toggleGroupDependentFields() {
        ...
        if (window.console && window.console.log) {
            console.log('Toggle fields for group:', groupName);
        }
        ...
        if (groupName && groupName.toLowerCase() === 'peers') {
            ...
            if (window.console && window.console.log) {
                console.log('Enabled fields for Peers group');
            }
        } else {
            ...
            if (window.console && window.console.log) {
                console.log('Disabled fields for non-Peers group');
            }
        }
    ...
    function updateAssessorCount() {
        ...
        if (window.console && window.console.log) {
            console.log(`Assessors selected: ${checked} of ${total}`);
        }
    }
...
<?php
if (WP_DEBUG) {
    error_log('Template render completed');
}
?>
