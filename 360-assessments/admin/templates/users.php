...
try {
    ...
    // Get current status and validate it
    ...
    if (WP_DEBUG) {
        error_log("Loading data for action: $current_action");
    ...
    if ($current_action === 'edit' || $current_action === 'new') {
        ...
        if (WP_DEBUG) {
            error_log('Data loaded:');
            error_log('Positions: ' . count($positions));
            error_log('Groups: ' . count($groups));
            error_log('Potential assessors: ' . count($potential_assessors));
            if ($user) {
