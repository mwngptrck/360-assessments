...
// If your main plugin file loads the handler, you do NOT need to require it again here.

// Check if plugin user is logged in
if (!Assessment_360_Auth_Handler::is_plugin_user_logged_in()) {
    wp_redirect(home_url('/360-assessment-login/'));
    exit;
}

// Get current plugin user (from plugin table/session)
$plugin_user = Assessment_360_Auth_Handler::get_plugin_user();

if (!$plugin_user) {
    // Session exists but user not found in DB (rare/corrupt session), force logout
    unset($_SESSION['assessment_360_user_id']);
    wp_redirect(home_url('/360-assessment-login/'));
    exit;
}

// Use plugin user info
$user_id = $plugin_user->id;
$user_first_name = $plugin_user->first_name;
$user_last_name = $plugin_user->last_name;
$user_email = $plugin_user->email;

// Get assessment IDs from URL if needed
...
// Initialize managers (assuming these are your plugin classes)
...
// Prevent self-assessment loop/refresh
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                    <input type="hidden" name="action" value="assessment_360_logout">
                    ...
                    <div class="col-md-7">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <?php
                                    $total_to_assess = is_array($users_to_assess) ? count($users_to_assess) : 0;
                                    $completed = 0;
                                    foreach ($users_to_assess as $assessee) {
                                        if ($assessment_manager->is_assessment_completed($user_id, $assessee->id, $active_assessment ? $active_assessment->id : null)) {
                                            $completed++;
                                        }
                                    }
                                    ...
                                    <div class="col-md-4">
                                        <div class="stat-item">
                                            ...
                                            <span class="stats-number">
                                                <?php echo esc_html($total_to_assess); ?>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
