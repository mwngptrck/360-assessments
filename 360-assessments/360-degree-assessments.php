...
function assessment_360_deactivate() {
    ...
    // Log deactivation if debugging
    if (WP_DEBUG) {
        error_log('360 Assessments plugin deactivated');
    }
}
...
function assessment_360_email_templates_page() { require_once ASSESSMENT_360_PLUGIN_DIR . 'admin/templates/email-templates.php'; }
...
function handle_save_position() {
    ...
    try {
        ...
        wp_redirect(add_query_arg(
            'message',
            urlencode($message),
            admin_url('admin.php?page=assessment-360-positions')
        ));
        exit;
...
function handle_restore_position() {
    ...
    if (is_wp_error($result)) {
        wp_redirect(add_query_arg('error', $result->get_error_message(), admin_url('admin.php?page=assessment-360-positions')));
    } else {
        wp_redirect(add_query_arg('message', 'Position restored successfully.', admin_url('admin.php?page=assessment-360-positions')));
    }
    exit;
}
...
function handle_save_group() {
    ...
    try {
        ...
        wp_redirect(add_query_arg(
            'message',
            urlencode($message),
            admin_url('admin.php?page=assessment-360-groups')
        ));
        exit;
