...
function assessment_360_activate($network_wide) {
    ...
    //$user_manager->verify_relationships_table(); // Add this line
...
// Login and logout
add_action('init', function() {
    // Login handler (your existing code)
...
// Add logout handler
add_action('init', 'handle_assessment_360_logout');

function handle_assessment_360_logout() {
    if (isset($_GET['action']) && $_GET['action'] === 'assessment_360_logout') {
        // Verify nonce
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'assessment_360_logout')) {
            wp_die('Invalid security token');
        }
...
function update_user_to_assessees_relationships($user_id, $assessees) {
    global $wpdb;

    // Remove old relationships for this user as assessor
    $wpdb->delete(
    ...
    foreach ($assessees as $assessee_id) {
        if ($assessee_id == $user_id) continue; // Don't assign self
        $wpdb->insert(
            $wpdb->prefix . '360_user_relationships',
            [
                'assessor_id' => $user_id,
...
function handle_save_user() {
    ...
    try {
        ...
        if (isset($_POST['assessees']) && is_array($_POST['assessees'])) {
            $assessees = array_map('intval', $_POST['assessees']);
            $result = $user_manager->update_user_assessees($user_id, $assessees);
        }
