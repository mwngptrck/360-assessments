...
function add_setup_page() {
    ...
    add_submenu_page(
        ...
        function() {
            ...
            require_once plugin_dir_path(__FILE__) . 'admin/templates/setup-wizard.php';
        }
    );
}
...
function handle_save_setup_settings() {
    ...
    check_admin_referer('save_setup_settings');

    $next_step = isset($_POST['next_step']) && !empty($_POST['next_step']) ? 
                 sanitize_text_field($_POST['next_step']) : 'groups';
...
function handle_save_setup_groups() {
    ...
    check_admin_referer('save_setup_groups');
...
function handle_save_setup_positions() {
    ...
    check_admin_referer('save_setup_positions');
...
function handle_delete_group() {
    ...
    if ($group && $group_manager->is_system_group($group->group_name)) {
        wp_redirect(add_query_arg([
            'page' => 'assessment-360-user-management',
            'tab' => 'groups',
            'error' => urlencode('System groups cannot be deleted.')
        ], admin_url('admin.php')));
        exit;
    }
