...
class Assessment_360_Group_Manager {
    ...
    public function verify_table_structure() {
        ...
        if (in_array('name', $column_names) && !in_array('group_name', $column_names)) {
            ...
            if (WP_DEBUG) {
                error_log('Renamed groups table column from name to group_name');
            }
        }
    }
    ...
    public function get_all_groups() {
        ...
        if (WP_DEBUG) {
            error_log('Getting all groups query: ' . $query);
        }

        $groups = $wpdb->get_results($query);

        if (WP_DEBUG) {
            error_log('Found groups: ' . print_r($groups, true));
        }
    ...
    public function handle_group_form() {
        ...
        wp_redirect(add_query_arg('message', urlencode($message), admin_url('admin.php?page=assessment-360-groups')));
        exit;
    }

    public function handle_group_delete() {
        if (!isset($_GET['action']) || $_GET['action'] !== 'delete' || 
            !isset($_GET['page']) || $_GET['page'] !== 'assessment-360-groups') {
            return;
        }
