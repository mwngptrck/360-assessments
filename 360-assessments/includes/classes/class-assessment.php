...
class Assessment_360_Assessment {
    ...
    public function create_assessment($data) {
        ...
        if (empty($data['name'])) {
            return new WP_Error('missing_name', 'Assessment name is required');
        }
        ...
        try {
            ...
            // Log the creation
            error_log('Assessment created: ' . print_r([
                'id' => $assessment_id,
                'name' => $data['name'],
                'status' => $data['status'] ?? 'active'
            ], true));
            ...
            // Rollback on error
            $wpdb->query('ROLLBACK');
            error_log('Assessment creation failed: ' . $e->getMessage());
            return new WP_Error('creation_failed', 'Failed to create assessment: ' . $e->getMessage());
        }
    }
    ...
    public function get_user_assessment_stats($user_id) {
        ...
        $total_to_assess = $wpdb->get_var($wpdb->prepare(
        ...
        $total_to_assess = (int)$total_to_assess;
    ...
    public function get_dashboard_stats($user_id) {
        ...
        // Get total assessors assigned to this user
        ...
        return (object)[
            ...
            'completion_rate' => $completion_rate,
            'assessment_id' => $active_assessment->id,
            'assessment_name' => $active_assessment->name
        ];
    }
    
    public function get_user_previous_results($user_id, $current_assessment_id = null) {
        global $wpdb;
        ...
        $query = $wpdb->prepare(
            "SELECT DISTINCT 
                ...
                (
                    ...
                    FROM {$wpdb->prefix}360_assessment_responses ar2
                    WHERE ar2.assessment_id = a.id
                    AND ar2.assessee_id = %d
                ) as total_assessors,
                (
                    SELECT COUNT(DISTINCT ar3.assessor_id)
                    FROM {$wpdb->prefix}360_assessment_responses ar3
                    WHERE ar3.assessment_id = a.id
                    AND ar3.assessee_id = %d
                    AND ar3.status = 'completed'
                ) as completed_assessments,
                (
                    SELECT AVG(ar4.rating)
                    FROM {$wpdb->prefix}360_assessment_responses ar4
                    WHERE ar4.assessment_id = a.id
                    AND ar4.assessee_id = %d
                ) as average_rating
            FROM {$wpdb->prefix}360_assessments a
            JOIN {$wpdb->prefix}360_assessment_responses ar 
        ...
        if ($current_assessment_id) {
            $query .= $wpdb->prepare(" AND a.id != %d", $current_assessment_id);
        }

        $query .= " ORDER BY a.created_at DESC LIMIT 3";

        $previous_assessments = $wpdb->get_results($query);

        if ($wpdb->last_error) {
            error_log('Database error in get_user_previous_results: ' . $wpdb->last_error);
            return array();
        }

        // Get detailed results for each assessment
        $results = array();
        foreach ($previous_assessments as $prev_assessment) {
            ...
            foreach ($questions as $question) {
                ...
                if (!isset($formatted_results[$topic_name])) {
                    $formatted_results[$topic_name] = array(
                        'sections' => array()
                    );
                }

                if (!isset($formatted_results[$topic_name]['sections'][$section_name])) {
                    $formatted_results[$topic_name]['sections'][$section_name] = array(
                        'questions' => array()
                    );
                }
                ...
                if ($question->comments) {
                    foreach (explode('||', $question->comments) as $comment) {
                ...
                $formatted_results[$topic_name]['sections'][$section_name]['questions'][] = array(
                    'id' => $question->question_id,
                    'text' => $question->question_text,
                    'average_rating' => round($question->average_rating ?? 0, 2),
                    'total_assessors' => $question->total_assessors,
                    'comments' => $comments
                );
            }

            $results[] = array(
                'assessment_id' => $prev_assessment->assessment_id,
                'assessment_name' => $prev_assessment->assessment_name,
                'created_at' => $prev_assessment->created_at,
                'total_assessors' => $prev_assessment->total_assessors,
                'completed_assessments' => $prev_assessment->completed_assessments,
                'average_rating' => round($prev_assessment->average_rating ?? 0, 2),
    ...
    public function get_user_all_assessment_results($user_id) {
        global $wpdb;
        ...
        // Get detailed results by topic
        foreach ($assessments as &$assessment) {
            $assessment->topics = $wpdb->get_results($wpdb->prepare(
                "SELECT 
                    t.name as topic_name,
                    AVG(ar.rating) as average_rating
                FROM {$wpdb->prefix}360_assessment_responses ar
    ...
    public function get_overall_completion_rate() {
        ...
        $result = $wpdb->get_row(
            "SELECT 
                COUNT(DISTINCT CASE WHEN status = 'completed' THEN id END) * 100.0 / 
        ...
        return round(floatval($result->completion_rate ?? 0), 2);
    }
    ...
    public function get_current_assessment() {
        ...
        if ($assessment) {
            ...
            if ($stats) {
                $assessment->total_assessors = (int)$stats->total_assessors;
                $assessment->completed_assessments = (int)$stats->completed_assessments;
                $assessment->completion_rate = $stats->total_assessors > 0 
                    ? round(($stats->completed_assessments / $stats->total_assessors) * 100, 2)
                    : 0;
            } else {
                $assessment->total_assessors = 0;
