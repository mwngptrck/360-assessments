...
class Assessment_360_Settings_Manager {
    ...
    public function init_settings() {
        ...
        // Only initialize if setup hasn't been done
    ...
    public function add_logo_to_login() {
        $org_name = (string) get_option('assessment_360_organization_name', '');
//        if ($logo_url) {
...
//        }
        ...
        if (!empty($org_name) && strpos($request_uri, 'assessment-360') !== false) {
            return $org_name . ' - 360° Assessment';
        }
        return $admin_title;
    }
    ...
    public function add_dashboard_header() {
        $org_name = (string) get_option('assessment_360_organization_name', '');
        $logo_url = (string) get_option('assessment_360_organization_logo', '');
        ?>
    ...
    public function assessment_360_handle_email_send() {
        ...
        foreach ($grouped_recipients as $group_name => $users) {
            ...
            foreach ($users as $user) {
                switch ($template) {
                    case 'welcome':
                        $subject = (string) get_option('assessment_360_welcome_email_subject', '');
                        $body = (string) get_option('assessment_360_welcome_email_body', '');
                        break;
