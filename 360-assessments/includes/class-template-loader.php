...
class Assessment_360_Template_Loader {
    ...
    public function load_template($template) {
        ...
        // Debug logging
        error_log('Template loader called');
        error_log('Current URL: ' . $_SERVER['REQUEST_URI']);
        error_log('Current post type: ' . ($post ? $post->post_type : 'no post'));
    ...
    private function load_dashboard_template() {
        ...
        if (file_exists($template)) {
            error_log('Loading dashboard template: ' . $template);
            return $template;
        }
    ...
    private function load_form_template() {
        ...
        if (file_exists($template)) {
            error_log('Loading form template: ' . $template);
            return $template;
        }
    ...
    private function load_login_template() {
        ...
        if (file_exists($template)) {
            error_log('Loading login template: ' . $template);
            return $template;
        }
