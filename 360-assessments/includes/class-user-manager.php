...
class Assessment_360_User_Manager {
    ...
    public function get_user_stats() {
        ...
        $stats->total_users = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}360_users");
        // Active users: those who've submitted any assessment in last 30 days
        $stats->active_users = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(DISTINCT assessor_id) FROM {$wpdb->prefix}360_assessment_responses WHERE completed_at >= %s",
            date('Y-m-d H:i:s', strtotime('-30 days'))
        ));
        // Last login (if tracked)
        $stats->last_login = $wpdb->get_var("SELECT MAX(last_login) FROM {$wpdb->prefix}360_users");
        return $stats;
    }
    ...
    public function update_user_relationships($user_id, $assessor_ids, $assessment_id) {
        global $wpdb;
        $relationships_table = $wpdb->prefix . '360_user_relationships';

        $wpdb->query('START TRANSACTION');
        try {
            // Delete existing relationships for this assessee
            $wpdb->delete(
                $relationships_table,
                ['assessee_id' => $user_id, 'assessment_id' => $assessment_id],
                ['%d', '%d']
            );
            if ($wpdb->last_error) {
            ...
            foreach ($assessor_ids as $assessor_id) {
                // Fetch the group_name of the assessor (non-department, active)
                $group = $wpdb->get_row(
                    $wpdb->prepare(
                        "SELECT g.group_name FROM {$wpdb->prefix}360_user_groups g
                         JOIN {$wpdb->prefix}360_users u ON u.group_id = g.id
                         WHERE u.id = %d AND g.is_department = 0 AND g.status = 'active' LIMIT 1",
                        $assessor_id
                    )
                );
                $relationship_type = $group ? $group->group_name : '';
    ...
    public function update_user_assessors($user_id, $assessor_ids) {
        ...
        try {
            ...
            $assessment_id = $this->get_current_active_assessment_id();
            if (!$assessment_id) {
            ...
            foreach ($assessor_ids as $assessor_id) {
                if ($assessor_id == $user_id) continue;
                // Insert into user_assessors
                $result = $wpdb->insert(
                    $wpdb->prefix . '360_user_assessors',
                    [
                        'user_id' => $user_id,
                ...
                // Insert into relationships with correct type
                $group = $wpdb->get_row(
                    $wpdb->prepare(
                        "SELECT g.group_name FROM {$wpdb->prefix}360_user_groups g
                         JOIN {$wpdb->prefix}360_users u ON u.group_id = g.id
                ...
                if ($wpdb->last_error) {
                    throw new Exception('Failed to insert into relationships: ' . $wpdb->last_error);
                }
            }
            $wpdb->query('COMMIT');
            return true;
    ...
    public function update_user_to_assessees_relationships($user_id, $assessees) {
        ...
        $assessment_id = $this->get_current_active_assessment_id();
        if (!$assessment_id) {
            return new WP_Error('missing_assessment', 'No active assessment found.');
        }
        // Delete only relationships for this assessor and this assessment
        $wpdb->delete(
            $wpdb->prefix . '360_user_relationships',
            [
                'assessor_id' => $user_id,
        ...
        foreach ($assessees as $assessee_id) {
            // Fetch the group_name of this assessor (non-department, active)
            $group = $wpdb->get_row(
                $wpdb->prepare(
                    "SELECT g.group_name FROM {$wpdb->prefix}360_user_groups g
                     JOIN {$wpdb->prefix}360_users u ON u.group_id = g.id
    ...
    public function get_current_active_assessment_id() {
        global $wpdb;
        return (int) $wpdb->get_var(
            "SELECT id FROM {$wpdb->prefix}360_assessments WHERE status = 'active' ORDER BY start_date DESC LIMIT 1"
        );
    }
    ...
    public function create_user_relationships() {
        ...
        try {
            ...
            // Get all active groups that are not departments
            $groups = $wpdb->get_results(
                "SELECT id, group_name FROM {$wpdb->prefix}360_user_groups WHERE is_department = 0 AND status = 'active'"
            );
            // Get current active assessment_id
            $assessment_id = $this->get_current_active_assessment_id();
            if (!$assessment_id) {
                throw new Exception('No active assessment found.');
            }
            foreach ($groups as $group) {
                $group_users = array_filter($users, function($user) use ($group) {
                    return $user->group_id == $group->id;
                });
                // Create relationships for this group (including self if desired)
                foreach ($group_users as $assessor) {
                    foreach ($group_users as $assessee) {
                        $wpdb->replace(
                            $wpdb->prefix . '360_user_relationships',
                            array(
                                ...
                                'assessment_id' => $assessment_id,
                                'relationship_type' => $group->group_name,
                                'created_at' => current_time('mysql')
                            ),
                            array('%d', '%d', '%d', '%s', '%s')
                        );
