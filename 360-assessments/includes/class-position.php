...
class Assessment_360_Position {
    ...
    public function create_position($data) {
        ...
        if (WP_DEBUG) {
            error_log('Creating position with data: ' . print_r($insert_data, true));
        }
        ...
        if ($result === false) {
            if (WP_DEBUG) {
                error_log('Failed to create position. DB Error: ' . $wpdb->last_error);
            }
    ...
    public function update_position($id, $data) {
        ...
        if (WP_DEBUG) {
            error_log('Updating position with data: ' . print_r($update_data, true));
        }
        ...
        if ($result === false) {
            if (WP_DEBUG) {
                error_log('Failed to update position. DB Error: ' . $wpdb->last_error);
            }
    ...
    public function handle_position_actions() {
        if (!isset($_GET['action']) || !isset($_GET['page']) || $_GET['page'] !== 'assessment-360-positions') {
            return;
        }
    ...
    public function verify_table_structure() {
        ...
        if (empty($description_exists)) {
            ...
            if (WP_DEBUG) {
                error_log('Added description column to positions table');
            }
        }
    }
