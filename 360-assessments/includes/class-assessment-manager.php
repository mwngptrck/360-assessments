...
class Assessment_360_Assessment_Manager {
    ...
    public function get_assessment($id) {
        ...
        try {
            ...
            if (WP_DEBUG) {
                error_log("Getting assessment: $query");
            }
...
//
//            if (WP_DEBUG) {
//                error_log('Getting assessment questions query: ' . $query);
//            }
//
...
//        } catch (Exception $e) {
//            if (WP_DEBUG) {
//                error_log('Error getting assessment questions: ' . $e->getMessage());
//            }
//            return array();
//        }
//    }
...
//    }
    ...
    public function get_user_statistics($user_id) {
        ...
        try {
            ...
            if (!$active_assessment) {
                if (WP_DEBUG) {
                    error_log('No active assessment found for user statistics');
                }
            ...
            if (!$user_id) {
                if (WP_DEBUG) {
                    error_log('Invalid user ID provided for statistics');
                }
                return $this->get_empty_statistics();
            }
            ...
            if (!$user_group || !isset($user_group->group_name)) {
                if (WP_DEBUG) {
                    error_log("User $user_id not found or has no group assigned");
                }
                return $this->get_empty_statistics();
            }

            if (WP_DEBUG) {
                error_log("User $user_id group info: " . print_r($user_group, true));
            }
            ...
            if (WP_DEBUG) {
                error_log("Statistics query for " . ($is_peer ? 'Peer' : 'Non-peer') . " user:");
                error_log($query);
            }
            ...
            if (!$stats) {
                if (WP_DEBUG) {
                    error_log("No statistics found for user $user_id");
                }
                return $this->get_empty_statistics();
            }
            ...
            if (WP_DEBUG) {
                error_log("Statistics calculated for user $user_id:");
                error_log("Total to assess: $total");
                error_log("Completed assessments: $completed");
                error_log("Pending assessments: $pending");
...
//    public function is_assessment_completed($assessment_id, $assessor_id, $assessee_id) {
//        global $wpdb;
//        
//        try {
//            $query = $wpdb->prepare(
...
//
//            if (WP_DEBUG) {
//                error_log('Checking assessment completion query: ' . $query);
//            }
//
...
//    }
    ...
    public function save_assessment_response($data) {
        ...
        if (WP_DEBUG) {
            error_log('Saving assessment response with data: ' . print_r($data, true));
        }
        ...
        try {
            ...
            if (WP_DEBUG) {
                error_log('Error saving assessment response: ' . $e->getMessage());
            }
    ...
    public function get_assessment_status($assessment_id, $assessor_id, $assessee_id) {
        global $wpdb;
        ...
        try {
            ...
            if (WP_DEBUG) {
                error_log('Error getting assessment status: ' . $e->getMessage());
            }
            return 'pending';
        }
    }
    ...
    public function save_assessment($data) {
        ...
        try {
            ...
            foreach ($data['ratings'] as $question_id => $rating) {
                $comment = isset($data['comments'][$question_id]) 
                    ? $data['comments'][$question_id] 
                    : '';
...
//
//            if (WP_DEBUG) {
//                error_log("Getting assessment stats for user $user_id");
...
//
//            if (WP_DEBUG) {
//                error_log("User assessment stats: " . print_r($result, true));
//            }
//
//            return $result;
//
//        } catch (Exception $e) {
//            if (WP_DEBUG) {
//                error_log('Error getting user assessment stats: ' . $e->getMessage());
//                error_log('Stack trace: ' . $e->getTraceAsString());
//            }
//
//            return (object)[
    ...
    public function get_user_assessment_stats($user_id) {
        ...
        try {
            ...
            if (!$current_assessment) {
                return $this->get_empty_user_stats();
            }

            // Get peers to assess
            $peers_query = "SELECT COUNT(DISTINCT u2.id) as total_peers
                FROM {$wpdb->prefix}360_users u1
                JOIN {$wpdb->prefix}360_user_groups g1 ON u1.group_id = g1.id
                JOIN {$wpdb->prefix}360_users u2 ON u2.group_id = g1.id
                WHERE u1.id = %d 
                AND u2.id != %d 
                AND u2.status = %s 
                AND g1.group_name = %s";
            ...
            $completed_query = "SELECT COUNT(DISTINCT ai.assessee_id) as completed
                FROM {$wpdb->prefix}360_assessment_instances ai
            ...
            // Calculate statistics
            $pending = $total_peers - $completed;
            $completion_rate = $total_peers > 0 ? round(($completed / $total_peers) * 100) : 0;

            if (WP_DEBUG) {
                error_log("User assessment stats for user $user_id:");
                error_log("Total peers to assess: $total_peers");
                error_log("Completed assessments: $completed");
                error_log("Pending assessments: $pending");
                error_log("Completion rate: $completion_rate%");
            }
    ...
    public function delete_assessment($id) {
        ...
        try {
            ...
            if ($assessment->status === 'completed') {
                throw new Exception('Completed assessments cannot be deleted');
            }

            if ($assessment->status === 'active') {
                throw new Exception('Active assessments cannot be deleted');
            }

            if (WP_DEBUG) {
                error_log("Starting deletion of assessment ID: $id");
            }

            // Start transaction
            $wpdb->query('START TRANSACTION');

            // First, get all assessment instances for this assessment
            $instance_ids = $wpdb->get_col($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}360_assessment_instances WHERE assessment_id = %d",
                $id
            ));

            if (WP_DEBUG) {
                error_log("Found " . count($instance_ids) . " assessment instances to delete");
            }
            ...
            if (!empty($instance_ids)) {
                ...
                if ($wpdb->last_error) {
                    throw new Exception("Error deleting responses: " . $wpdb->last_error);
                }

                if (WP_DEBUG) {
                    error_log("Deleted responses for instances: $instance_ids_string");
                }
            }
            ...
            if ($wpdb->last_error) {
                throw new Exception("Error deleting assessment instances: " . $wpdb->last_error);
            }

            if (WP_DEBUG) {
                error_log("Deleted assessment instances for assessment ID: $id");
            }
            ...
            if ($wpdb->last_error) {
                throw new Exception('Error deleting assessment: ' . $wpdb->last_error);
            }

            if (WP_DEBUG) {
                error_log("Successfully deleted assessment ID: $id");
            }
            ...
            $wpdb->query('ROLLBACK');
            if (WP_DEBUG) {
                error_log('Error in delete_assessment: ' . $e->getMessage());
                error_log('Stack trace: ' . $e->getTraceAsString());
            }
